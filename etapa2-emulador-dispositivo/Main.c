...
void iniciarlog() {
    ...
    if (log_file == NULL) {
        ...
        escreverlog("Erro ao abrir o arquivo de log: %s", strerror(errno));
        exit(EXIT_FAILURE);
    }
...
int save_config(const char *filename, const Config *configs, int device_count) {
    ...
    if (!file) {
        ...
        escreverlog("Erro ao abrir o arquivo para leitura: %s", strerror(errno));
        return 0;
    }
    ...
    if (!file) {
        ...
        escreverlog("Erro ao abrir o arquivo para escrita: %s", strerror(errno));
        cJSON_Delete(json);
        return 0;
    }
    ...
    if (!json_string) {
        ...
        escreverlog("Erro ao converter JSON para string: %s", strerror(errno));
        cJSON_Delete(json);
        fclose(file);
        return 0;
    }
...
int send_to_ttn(const uint8_t *packet, int packet_len) {
    ...
    if (inet_pton(AF_INET, "127.0.0.1", &server_addr.sin_addr) <= 0) {
        ...
        escreverlog("Erro ao configurar o endereço do servidor: %s", strerror(errno));
        close(sockfd);
        return -1;
    }
    ...
    if (sendto(sockfd, packet, packet_len, 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        ...
        escreverlog("Erro ao enviar pacote: %s", strerror(errno));
        close(sockfd);
        return -1;
    }
